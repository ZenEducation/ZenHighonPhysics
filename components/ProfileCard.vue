<script>
  ...
  export default {
	...
	methods:{
		changeTheme(){
		if (this.theme) {
			this.$refs.btn.innerHTML = "☀️";
			this.$refs.body.classList.remove('text-gray-900');
			this.$refs.body.classList.add('text-gray-100');
			this.$refs.profile.classList.remove('bg-white');
			this.$refs.profile.classList.add('bg-gray-900');
		} else{
			this.$refs.btn.innerHTML = "🌙";
			this.$refs.body.classList.remove('text-gray-100');
			this.$refs.body.classList.add('text-gray-900');
			this.$refs.profile.classList.remove('bg-gray-900');			
			this.$refs.profile.classList.add('bg-white');
		  }
...
<template>
    <div ref="body" class="box-border h-screen w-screen font-sans antialiased text-gray-900 leading-normal tracking-wider bg-cover " style="background-image:url('https://source.unsplash.com/1L71sPT5XKc');">
    <div class="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto py-8 lg:my-0">
    
	<!--Main Col-->
	<div ref="profile" class="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white opacity-75 mx-6 lg:mx-0">
	...
	<!-- Pin to top right corner -->
	  <div class="absolute top-0 right-0 h-12 w-18 p-4">
		<button ref="btn" class="js-change-theme focus:outline-none" @click="changeTheme()">🌙</button>
	  </div>
